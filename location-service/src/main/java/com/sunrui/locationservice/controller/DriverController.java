...
public class DriverController {
    ...
    public ResponseEntity<Driver> create(@RequestBody(required = false) Driver driver) {
        ...
        return new ResponseEntity<>(driver, HttpStatus.CREATED); //201
    }

    @RequestMapping(value="/drivers", method = RequestMethod.GET)
    public ResponseEntity<List<Driver>> getAll() {
        return new ResponseEntity<>(new ArrayList<>(drivers.values()), HttpStatus.OK); //http status returns 200
    }
    ...
    public ResponseEntity<Driver> get(@PathVariable("id") String id) {
        ...
        if(!drivers.containsKey(id)) {
            return new ResponseEntity<>(driver, HttpStatus.BAD_REQUEST); //if does't have the id, http status returns 400
        }
